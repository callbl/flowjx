...
import { ArduinoCodeEditor } from "../arduino-code-editor";
...
export function ArduinoUnoNode({
  ...
  const getPinState = (pinId: string): "HIGH" | "LOW" | number => {
    return (data.pinStates?.get(pinId) as "HIGH" | "LOW" | number) || "LOW";
  };

  // Check if digital pin is HIGH
  const isPinHigh = (pinId: string): boolean => {
    const state = getPinState(pinId);
    return state === "HIGH" || (typeof state === "number" && state > 127);
  };
    ...
    <>
      ...
      >
        ...
        <Button
          ...
          size="icon-sm"
          variant="secondary"
          className="absolute top-2 left-2 z-10 nodrag shadow-md"
        ...
        {/* Running Indicator */}
        {isRunning && (
          <div className="absolute top-2 right-2 z-10 flex items-center gap-1.5 px-2 py-1 bg-green-500/90 text-white text-xs font-medium rounded shadow-md">
        ...
        >
          ...
          {/* LEDs */}
          {/* Power LED (Green - always on when running) */}
          ...
          {/* L LED (Pin 13) - lights up when D13 is HIGH */}
            ...
            return (
              <g key={pin.id}>
                <rect
                  ...
                  width="14"
                  height="6"
                  fill={isHigh ? "#FFD700" : `url(#goldGradient-${id})`}
                  rx="0.5"
                />
                <rect
                  x={BOARD_WIDTH - 24}
          ...
          return (
            <Handle
              ...
              style={{
                ...
                width: 8,
                height: 8,
                background: isHigh ? "#FF8C00" : "#DAA520",
                border: `2px solid ${isHigh ? "#FFD700" : "#B8860B"}`,
                borderRadius: 2,
                boxShadow: isHigh ? "0 0 8px #FFD700" : "none",
              }}
            />
          );
        })}
