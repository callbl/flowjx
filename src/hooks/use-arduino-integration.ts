...
export function useArduinoIntegration() {
   ...
   */
  const findConnectedComponents = useCallback((mcuNodeId: string, pinId: string, currentNodes: Node[]): Node[] => {
   ...
   */
  const updateComponentFromPin = useCallback((
    node: Node,
    pinState: PinRuntimeState,
    currentUpdateNodeData: <T>(nodeId: string, data: Partial<T>) => void
  ) => {
  ...
  }, [findConnectedComponents, updateComponentFromPin]);
  ...
  }, [runtime, isRunning, handlePinChange]);

  /**
   * Find MCU pins connected to a component
   */
  const findConnectedPins = useCallback((mcuNodeId: string, componentNodeId: string): string[] => {
    const edges = useCircuitStore.getState().edges;
    const pins: string[] = [];
   ...
   */
  const updateArduinoPin = useCallback((pinId: string, componentType: string, data: Record<string, unknown>) => {
    if (!runtime) return;
    ...
    switch (componentType) {
      ...
      case "light-sensor": {
        ...
        if (data.rawValue !== undefined) {
          ...
          const pinValue = Math.round((Number(data.rawValue) / 1023) * 255);
      ...
      case "ultrasonic-sensor": {
        ...
        if (data.distance !== undefined) {
          ...
          const rawValue = Math.min(1023, Math.round((Number(data.distance) / 400) * 1023));
      ...
      case "potentiometer": {
        ...
        if (data.value !== undefined) {
          const pinValue = Math.round((Number(data.value) / 1023) * 255);
          runtime.setExternalPinState(pinId, pinValue);
        }
        break;
      }
  ...
  useEffect(() => {
    ...
    currentNodes.forEach((node) => {
      ...
      connectedPins.forEach((pinId) => {
        if (pinId && type && data) {
          updateArduinoPin(pinId, type, data as Record<string, unknown>);
        }
      });
    });
  }, [runtime, isRunning, findConnectedPins, updateArduinoPin]);
}
